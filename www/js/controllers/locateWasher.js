app.controller('LocateWasherCtrl', function ($ionicPlatform, $scope, $rootScope, $cordovaDialogs, $window, $http, apiUrl, $state, $ionicLoading, store, $timeout, $ionicSideMenuDelegate, apiUsername, apiPassword, $stateParams, $interval, $cordovaSms, $cordovaGeolocation, PaypalService) {
  $scope.map = null;
  $scope.clientDetails = {
    marker: null,
    latLng: null,
  ...
  $scope.goToWasherOngoingJob = function () {
    ...
    $.ajax({
      ...
      console.log("updaterequeststatus goToWasherOngoingJob response :"+JSON.stringify(data2));
  ...
  $scope.getClientRequest = function (id) {
    ...
    $.ajax({
      ...
      if (data && data.code == 200) {
        ...
        $scope.job_cancel_status = data.response.job_cancel_status;
