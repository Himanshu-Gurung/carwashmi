app.controller('RatingWasherCtrl', function ($scope, $rootScope, $cordovaDialogs, $http, apiUrl, $state, $ionicLoading, store, apiUsername, apiPassword, $stateParams, PaypalService) {
  ...
  $scope.saveCreditCard = function () {
    $cordovaDialogs.alert('Credit Card Details Saved!');
    $state.go('home');
  }
  ...
  $scope.setCompletedStatus = function () {
    console.log("setCompletedStatus !!!");
    $ionicLoading.show();
    ...
    data.user_id = store.get("userId");
    data.number = '9';
    data.customer_rating = $scope.rating;
    if($scope.rate.remarks != undefined){
      data.customer_comment = $scope.rate.remarks;  
    }
    else
    {
    ...
    $.ajax({
      ...
      data.message = 'Washer gave you rating of ' + $scope.rating;
      $.ajax({
        type: 'POST',
        url: apiUrl,
        data: data,
